...
import dev.zenhao.melon.module.modules.crystal.PlaceInfo
...
import melon.utils.block.BlockUtil.getAnchorBlock
import melon.utils.block.BlockUtil.getNeighbor
import melon.utils.chat.ChatUtil
import melon.utils.combat.TargetInfo
...
import melon.utils.math.DamageCalculator.anchorDamage
...
object AnchorAura : Module(
    ...
    private val silentMode by msetting("SilentMode", SilentMode.Pick)
    ...
    init {
        ...
        safeConcurrentListener<RunGameLoopEvent.Tick> {
            runCatching {
                ...
                val anchorSlot = (if (silentMode != SilentMode.InvPick) player.hotbarSlots.firstItem(Items.RESPAWN_ANCHOR)
                    else player.allSlots.firstItem(Items.RESPAWN_ANCHOR))?.let { HotbarSlot(it) } ?: return@safeConcurrentListener
                val glowSlot = (if (silentMode != SilentMode.InvPick) player.hotbarSlots.firstItem(Items.GLOWSTONE)
                    else player.allSlots.firstItem(Items.GLOWSTONE))?.let { HotbarSlot(it) } ?: return@safeConcurrentListener
    ...
    private fun SafeClientEvent.globalPlace(slot: HotbarSlot, placeInfo: PlaceInfo, explode: Boolean) {
        if (explode) {
            ...
            player.spoofSneak {
                ...
                when (silentMode) {
                    SilentMode.Pick -> spoofHotbar(slot) {
                        sendPacket()
                    }

                    SilentMode.Swap -> spoofHotbarBypass(slot) {
                        sendPacket()
                    }

                    SilentMode.InvPick -> spoofInvBypass(slot) {
                        sendPacket()
                    }
                }
            }
